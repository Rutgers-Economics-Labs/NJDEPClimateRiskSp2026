...
def load_and_prepare():
    ...
    pre = df[df['is_post_2023'] == 0].groupby('muni_name').agg(
        spread_pre   = ('spread_bps_w', 'mean'),
        ttm_pre      = ('time_to_maturity', 'mean'),
        n_trades_pre = ('spread_bps_w', 'count'),
    )
    post = df[df['is_post_2023'] == 1].groupby('muni_name').agg(
...
def run_aipw():
    ...
    # Parsimonious: only 4 covariates for the small treated sample
    ...
    print(
        f"{'TWFE DiD (Model B)':<25} {TWFE_MODEL_B['coef']:>+12.2f} "
        f"{TWFE_MODEL_B['se']:>10.2f} {TWFE_MODEL_B['ci']:>25} {TWFE_MODEL_B['p']:>10}"
    )
    ...
    if sig_ok and sign_consistent and overlap_ok:
        ...
        print(f"  However, with only {n_champ} treated units, external validity is limited.")
    else:
        reasons = []
        if not sig_ok:
