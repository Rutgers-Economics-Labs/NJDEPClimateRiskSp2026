...
def process_ufb_debt():
    ...
    for f in files:
        ...
        # We need Municipality, Net Debt, Assessed Value (or Property Valuation)
        ...
        if not col_muni or not col_net_debt or not col_assessed_val:
            print(f"  WARNING: Missing columns in {year}. (M:{col_muni}, ND:{col_net_debt}, AV:{col_assessed_val})")
            continue
            
        df_subset = df[[col_muni, col_net_debt, col_assessed_val]].copy()
        ...
        # Keep final columns
        df_final = df_subset[['municipality', 'ufb_year', 'net_debt', 'assessed_value', 'debt_to_assessed_value']]
        all_years.append(df_final)

    if all_years:
        combined = pd.concat(all_years, ignore_index=True)
        # Drop duplicate municipality/year pairs just in case
        combined = combined.drop_duplicates(subset=['municipality', 'ufb_year'])
